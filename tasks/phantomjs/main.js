...
setInterval(function() {
  if (new Date() - last > options.timeout) {
	var senchaExtdone = function() {
		var allScripts = document.querySelectorAll('script');
		var extfiles = Ext.Loader.history;
		var file, allfiles = [], historyfiles = [];
		for (var i=0; i<allScripts.length ;i++ )
		{
			file = allScripts[i].src.split('?');
			allfiles.push(file[0]);
		}
		for (var i=0; i<extfiles.length; i++)
		{
			file = Ext.Loader.getPath(extfiles[i]);
			historyfiles.push(file);
		}
		sendMessage('extTask.done', {
			scriptTags : allfiles,
			extfiles : historyfiles
		});
	}
	page.evaluate(senchaExtdone);
